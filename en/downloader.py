...
async def search_fetch():
    ...
    async def subtask(i):
        ...
        while 1:
            try:
                resp, body = await fetch(f'https://www.wikihow.com/wikiHowTo?search={i}&start={ctr}', return_resp=True)
            ...
            if resp.status != 200:
                ...
                break
            for i in re.findall(r'class="result_link" href=https://www.wikihow.com/(.*?) >', body):
...
async def random_fetch():
    ...
    async def random_fetch1():
        try:
            resp, html = await fetch('https://www.wikihow.com/Special:Randomizer', return_resp=True)
        ...
        if not c:
            ...
            await collection_paths.insert_one({
                '_id': resp.url.name, 'html': html, 
...
async def cache_fetch(path, forced_fetch=False):
    try:
        resp, cont = await fetch('https://www.wikihow.com/' + path, return_resp=True)
        if rate_limit_token in cont:
            # print('limit triggered')
            raise NameError('触发反爬')
        if resp.status == 404:
            # await collection_paths.delete_one({'_id': path})
            await collection_paths.find_one_and_update({'_id': path}, {'$set': {'t404': True}})
            raise NameError(f'找不到页面:{path}')
            
        if resp.status != 200:
            print(path, resp.status)
            raise NameError(f'非200码：{resp.status}')
        rp = path
        if hasattr(resp, 'url'):
            rp = resp.url.name
            if rp != path:
...
async def main():
    tasks = []
    cur = collection_paths.find({'html': {'$exists': False}, 't404': {'$exists': False}}) #.to_list(length=None)
    cid = 0
    async for x in cur:
        cid += 1
